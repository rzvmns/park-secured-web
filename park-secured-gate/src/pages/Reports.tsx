...
export default function Reports() {
  ...
  const handleExportCSV = () => {
    window.location.href = `${import.meta.env.VITE_API_BASE_URL || "https://park-secured-cloud-r62j.onrender.com/api"}/export-csv`;
  };
  ...
  const maxMonthly = Math.max(...reports.monthly.map((item: any) => item.value), 1);
      ...
      <section className="report-layout" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px" }}>
        ...
        <div className="card">
          ...
          <div className="department-list" style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
            {reports.byDepartment?.map((item: any) => (
