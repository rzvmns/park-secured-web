...
import { useEffect, useMemo, useState } from "react";import EmployeeTable from "../components/EmployeeTable";
import { useAuth } from "../context/AuthContext";
import { getEmployees, saveEmployee, request, getIndividualReport } from "../services/api";
...
function generateEmail(firstName: string, lastName: string, existingEmails: string[] = []): string {
...
function ModalAngajat({ editing, onClose, onSaved, userRole, userDivisionId }: { editing: any; onClose: any; onSaved: any; userRole: any; userDivisionId: any }) {
  ...
  useEffect(() => {
    ...
    request("/users").then((users) => {
      setExistingEmails((users || []).map((u: any) => u.email).filter(Boolean));
    }).catch(() => {});
  }, [isNew]);

  const updateEmailPreview = (firstName: string, lastName: string) => {
    ...
    try {
      ...
      if (isNew && emailCont) {
        ...
        try {
          ...
          setError(`Angajat creat, dar contul nu a putut fi creat: ${(err as any).message}`);
          setSaving(false);
          onSaved(saved);
          return;
        }
      ...
      setError((err as any).message || "Eroare la salvare.");
    } finally {
      setSaving(false);
    }
  };
        ...
        <form className="form-grid" onSubmit={handleSubmit}>
          <label>
            ...
            <input
              ...
              required
              onChange={(e) => updateEmailPreview(e.target.value, (document.querySelector('[name="lastName"]') as HTMLInputElement)?.value)}
            />
          </label>
          ...
          <label>
            ...
            <input
              ...
              required
              onChange={(e) => updateEmailPreview((document.querySelector('[name="firstName"]') as HTMLInputElement)?.value, e.target.value)}
            />
          </label>
...
function ModalRaport({ employee, onClose }: { employee: any; onClose: any }) {
  ...
  useEffect(() => {
    ...
    getIndividualReport(employeeId)
      .then(setReport)
      .catch((err: any) => setError(err.message || "Eroare la generarea raportului."))
        ...
        {report && !loading && (
          <>
            ...
            {report.events.length === 0 ? (
              ...
              <div className="table-wrap" style={{ maxHeight: 320, overflowY: "auto" as const }}>
                <table>
                  <thead>
                    <tr>
                      <th>Data / Ora</th>
...
export default function Employees() {
  ...
  const filteredEmployees = useMemo(() => {
    ...
    return employees.filter((employee) => {
      const areDreptulPeDivizie = user?.role === "admin" || user?.role === "hr" || Number((employee as any).divisionId) === Number((user as any)?.divisionId);
    ...
    <div className="page-grid">
      ...
      {editing && (
        <ModalAngajat
          ...
          onSaved={handleSaved}
          userRole={user?.role}
          userDivisionId={(user as any)?.divisionId}
        />
      )}
