...
export function setSession({ token, user }: { token: string; user: any }) {
...
export async function request(path: string, options: any = {}) {
  ...
  if (!response.ok) {
    const error = new Error(payload?.message || `API request failed: ${response.status}`) as Error & { status?: number; payload?: unknown };
...
function buildSchedule(start: string | null | undefined, end: string | null | undefined): string {
...
function generatedCnp(employee: any): string {
  return employee.cnp || `TMP${employee.id || Date.now()}`;
}

function mapCloudUser(user: any) {
  return {
...
function mapCloudEmployee(employee: any) {
...
function toCloudEmployeePayload(employee: any) {
...
function eventTypeFromDirection(direction: string): string {
  return direction === "OUT" ? "EXIT" : "ENTRY";
}

function createLocalLog({ employee, authorized, direction = "IN", method = "Portar", note = "" }: { employee: any; authorized: any; direction?: string; method?: string; note?: string }) {
  const log = {
    id: Date.now(),
...
export async function getAccessLogs() {
  try {
    ...
    return events.map((event: any) => mapCloudEvent(event, employees));
  } catch (error) {
    await wait();
...
export async function resolveAccessEvent(eventId: any, resolution: any) {
...
// ── Funcții exportate suplimentare ──────────────────────────────────────────

export async function getDeviceChangeRequests() {
  return request("/device-change-requests");
}

export async function resolveDeviceChangeRequest(requestId: any, status: any) {
...
export async function saveEmployee(employee: any) {
  if (employee.id) {
    return request(`/employees/${employee.id}`, {
      method: "PUT",
...
export async function getIndividualReport(employeeId: any) {
  return request(`/employees/${employeeId}/report`);
}
