...
const IconPerson = () => (
  <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
    ...
    <path d="M4 20c0-4 3.582-7 8-7s8 3 8 7" stroke="#9ca3af" strokeWidth="1.8" strokeLinecap="round"/>
  </svg>
);

function EmployeeAvatar({ name, photoUrl }: { name?: string; photoUrl?: string }) {
  const wrapRef = useRef<HTMLDivElement>(null);

  if (photoUrl) {
    return (
      <div ref={wrapRef} style={avatarBase}>
        <img
          src={photoUrl}
          alt={name}
...
export default function AccessLogTable({ logs = [] }: { logs?: any[] }) {
        ...
        <tbody>
          {logs.map((log) => (
            <tr key={log.id || Math.random()}>
              ...
              <td>
                <span className={`badge ${statusClass[log.status as keyof typeof statusClass] || "info"}`}>
